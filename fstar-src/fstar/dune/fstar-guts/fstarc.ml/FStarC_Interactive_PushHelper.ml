  ...
  fun this_fname ->
    fun deps ->
      fun table ->
        ...
        let loaded_mods_set =
          let uu___ = FStarC_PSMap.psmap_empty () in
          FStarC_List.fold_left
            (fun acc ->
               fun dep ->
                 let uu___1 = FStarC_Parser_Dep.lowercase_module_name dep in
                 FStarC_PSMap.psmap_add acc uu___1 true) uu___ deps in
        let loaded modname =
          FStarC_PSMap.psmap_find_default loaded_mods_set modname false in

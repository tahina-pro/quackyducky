...
let (set_option : Prims.string -> option_val -> unit) =
  fun k ->
    fun v ->
      ...
      if k = "report_assumes"
      then
        let uu___ = FStarC_PSMap.psmap_try_find map k in
        match uu___ with
        | FStar_Pervasives_Native.Some (String "error") -> ()
        | uu___1 ->
            let uu___2 = FStarC_PSMap.psmap_add map k v in
            FStarC_Effect.op_Colon_Equals fstar_options uu___2
      else
        (let uu___1 = FStarC_PSMap.psmap_add map k v in
...
let (init : unit -> unit) =
  fun uu___ ->
    ...
    (let uu___4 = FStarC_PSMap.psmap_empty () in
...
let (get_option : Prims.string -> option_val) =
  fun s ->
    let uu___ = let uu___1 = peek () in FStarC_PSMap.psmap_try_find uu___1 s in
    match uu___ with
              ...
              (fun k ->
                 ...
                 then Obj.magic (Obj.repr [])
                   ...
                   Obj.magic
                     (Obj.repr
                        (let v =
                           let uu___3 = FStarC_PSMap.psmap_try_find s k in
...
let (set_verification_options : optionstate -> unit) =
  fun o ->
    ...
    FStarC_List.iter
      (fun k ->
         let uu___ =
           let uu___1 = FStarC_PSMap.psmap_try_find o k in
...
let (parse_settings :
  ...
  =
  fun ns ->
    let cache = FStarC_SMap.smap_create (Prims.of_int (31)) in
    let with_cache f s =
      let uu___ = FStarC_SMap.smap_try_find cache s in
      match uu___ with
      | FStar_Pervasives_Native.Some s1 -> s1
      | FStar_Pervasives_Native.None ->
          let res = f s in (FStarC_SMap.smap_add cache s res; res) in

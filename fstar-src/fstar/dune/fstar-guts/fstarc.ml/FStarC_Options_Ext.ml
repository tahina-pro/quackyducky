...
type ext_state =
  | E of Prims.string FStarC_PSMap.psmap 
let (uu___is_E : ext_state -> Prims.bool) = fun projectee -> true
let (__proj__E__item__map : ext_state -> Prims.string FStarC_PSMap.psmap) =
  fun projectee -> match projectee with | E map -> map
let (defaults : (Prims.string * Prims.string) Prims.list) =
  [("context_pruning", "true")]
let (init : ext_state) =
  let uu___ =
    let uu___1 = FStarC_PSMap.psmap_empty () in
    FStarC_List.fold_right
      (fun uu___2 ->
         fun m -> match uu___2 with | (k, v) -> FStarC_PSMap.psmap_add m k v)
      defaults uu___1 in
  E uu___
    ...
    fun v ->
      let uu___ =
        let uu___1 =
          ...
          FStarC_PSMap.psmap_add uu___2 k v in
        E uu___1 in
  ...
  fun k ->
    let r =
      let uu___ =
        ...
        FStarC_PSMap.psmap_try_find uu___1 k in
      match uu___ with
...
let (getns : Prims.string -> (key * value) Prims.list) =
  fun ns ->
    ...
    FStarC_PSMap.psmap_fold uu___ f []
let (all : unit -> (key * value) Prims.list) =
  fun uu___ ->
    ...
    let uu___1 =
      let uu___2 = FStarC_Effect.op_Bang cur_state in
      __proj__E__item__map uu___2 in
    FStarC_PSMap.psmap_fold uu___1 f []

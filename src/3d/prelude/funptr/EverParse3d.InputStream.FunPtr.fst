module EverParse3d.InputStream.FunPtr
...
noextract
let has
    (#[FStar.Tactics.Typeclasses.tcresolve ()] aux_has: Aux.has_t)
    (x: t)
    (_: unit)
    (position: LPE.pos_t)
    (n: U64.t)
...
noextract
let read0
    (#[FStar.Tactics.Typeclasses.tcresolve ()] aux_read: Aux.read_t)
    (x: t)
    (position: LPE.pos_t)
    (n: U64.t)
    (dst: B.buffer U8.t)
...
= let dst = aux_read x.Aux.base n dst in
  let h1 = HST.get () in
...
noextract
let skip
    (#[FStar.Tactics.Typeclasses.tcresolve ()] aux_skip: Aux.skip_t)
    (x: t)
    (position: LPE.pos_t)
    (n: U64.t)
:   HST.Stack unit
...
let empty
    (#[FStar.Tactics.Typeclasses.tcresolve ()] aux_skip: Aux.skip_t)
    (#[FStar.Tactics.Typeclasses.tcresolve ()] aux_empty: Aux.empty_t)
    (x: t)
    (_: unit)
    (position: LPE.pos_t)
:   HST.Stack LPE.pos_t
...
=
  ...
  then begin
    ...
    aux_skip x.Aux.base (x.Aux.length `U64.sub` position);
    let h2 = HST.get () in
    ...
    let skipped = aux_empty x.Aux.base in
    let h2 = HST.get () in
...
= assert (live y h);
  assert (get_read y h `Seq.equal` get_read x h);
  assert (get_remaining y h `Seq.equal` (get_remaining x h `Seq.append` get_suffix x y))
#pop-options

...
(* Layer-2 (raw/) interface of the nondeterministic-CBOR structural map-entry
   ...
   and [friend]s [CBOR.Pulse.API.Nondet.Type] (and
   [CBOR.Pulse.Raw.Format.MixedList]) so that, inside it,
     [cbor_nondet_map_entry_insert_cell_t == IT.mixed_list cbor_map_entry],
     [cbor_nondet_map_entry_t == cbor_map_entry],  and  [cbor_nondet_t == cbor_raw].
...
(* Insert a (key, value) entry into a NONdeterministic-CBOR map [x] (whose spec
   value [y] satisfies [CMap? (unpack y)]).  The entry is PREPENDED (the
   nondeterministic encoding does not require sorted keys), so the abstract
   result is the union of the original map with the singleton {vk -> vv}.
   ...
   original map with the singleton {vk -> vv}.  Exposed via the EXISTENTIAL
   form (see the .fst for the rationale): we existentially quantify
   [vres : Spec.cbor] and assert that [CMap? (unpack vres)] holds and that

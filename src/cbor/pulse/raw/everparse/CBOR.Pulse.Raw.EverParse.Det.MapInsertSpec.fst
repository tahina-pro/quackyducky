              ...
              Spec.cbor_map_union (Spec.CMap?.c (Spec.unpack y)) (Spec.cbor_map_singleton vk vv)))
{
  ...
  assert (pure (L.sorted (Valid.map_entry_order MapLexInsert.order0 _)
                  (SpecRawBase.Map?.v (SpecRaw.mk_det_raw_cbor (Ghost.reveal y))) == true));
  (* Expose the raw [cbor_match] resources for the verified raw core. *)
  unfold (cbor_det_match p x y);
  unfold (cbor_det_match pkv key vk);
  ...
  match res {
    ...
    Some m -> {
      with pm_result xh_result. assert (
        ...
        Trade.trade
          ...
          (RawMatch.cbor_match p x (SpecRaw.mk_det_raw_cbor (Ghost.reveal y)) **
           RawMatch.cbor_match pkv key (SpecRaw.mk_det_raw_cbor (Ghost.reveal vk)) **
           RawMatch.cbor_match pkv value (SpecRaw.mk_det_raw_cbor (Ghost.reveal vv)) **
           (exists* w1 w2 w3 w4 wy. R.pts_to r1 w1 ** R.pts_to r2 w2 ** R.pts_to r3 w3 ** R.pts_to r4 w4 ** R.pts_to ry wy))
      );
      ...
      rewrite (RawMatch.cbor_match pm_result m xh_result)
        as (RawMatch.cbor_match pm_result m (SpecRaw.mk_det_raw_cbor (Ghost.reveal vres)));
      ...
      Trade.intro_trade
        ...
        fn _ {
          ...
          rewrite (RawMatch.cbor_match pm_result m (SpecRaw.mk_det_raw_cbor (Ghost.reveal vres)))
            as (RawMatch.cbor_match pm_result m xh_result);
          Trade.elim
            (RawMatch.cbor_match pm_result m xh_result)
            (RawMatch.cbor_match p x (SpecRaw.mk_det_raw_cbor (Ghost.reveal y)) **
             RawMatch.cbor_match pkv key (SpecRaw.mk_det_raw_cbor (Ghost.reveal vk)) **

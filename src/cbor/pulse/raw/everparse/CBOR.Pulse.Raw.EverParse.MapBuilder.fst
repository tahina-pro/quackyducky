...
requires
  I.mixed_list_match cbor_match_map_entry
    IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) pm ml (Ghost.reveal l)
returns y: cbor_raw
ensures
  ...
  pure (CBOR_Case_Map_Gen? y)
{
  I.mixed_list_match_length cbor_match_map_entry
    IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) pm ml (Ghost.reveal l);
  let len64 = IT.mixed_list_length IO.u64_ops ml;
  ...
  rewrite (I.mixed_list_match cbor_match_map_entry
    ...
    as (I.mixed_list_match cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item)
...
(*                                                                  *)
(* Mirrors [cbor_mk_map_full] but stamps the caller-chosen (possibly *)
(* non-minimal) length header [len], as in                          *)
(* [ArrayBuilder.cbor_array_finalize_with_len].                     *)
(* ================================================================ *)

#restart-solver
#push-options "--z3rlimit 20 --fuel 2 --ifuel 2"
fn cbor_mk_map_full_with_len
  (pm: perm)
  (ml: IT.mixed_list U64.t cbor_map_entry)
  (len: raw_uint64)
  ...
  cbor_map_finalized_val pm ml y (Map len (Ghost.reveal l))
{
  I.mixed_list_match_length cbor_match_map_entry
    IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) pm ml (Ghost.reveal l);
  let ct : cbor_mixed_list_map = {
    cbor_map_gen_length_size = len.size;
    cbor_map_gen_ptr = ml;
    cbor_map_gen_perm = pm;
  };
  let xh0 : Ghost.erased (r: raw_data_item { Map? r }) =
  ...
  Trade.rewrite_with_trade
    (cbor_match_mixed_list_map 1.0R ct (Ghost.reveal xh0) cbor_match)
    (cbor_match 1.0R y (Ghost.reveal xh0));
  assert (pure (ct.cbor_map_gen_perm == pm));
  assert (pure (ct.cbor_map_gen_ptr == ml));
  Trade.intro_trade
    (cbor_match_mixed_list_map 1.0R ct (Ghost.reveal xh0) cbor_match)
    (I.mixed_list_match cbor_match_map_entry
       IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) pm ml (Ghost.reveal l))
  ...
  rewrite (Trade.trade (cbor_match 1.0R y (Ghost.reveal xh0))
    ...
    as (Trade.trade (cbor_match 1.0R y (Map len (Ghost.reveal l)))
          (I.mixed_list_match cbor_match_map_entry
             IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) pm ml
             (Map?.v (Map len (Ghost.reveal l)))));
  fold (cbor_map_finalized_val pm ml y (Map len (Ghost.reveal l)));
  y
}
#pop-options
...
returns ml: IT.mixed_list U64.t cbor_map_entry
    ...
    (cbor_match pm (CBOR_Case_Serialized_Map v) (Ghost.reveal xh))
{
  ...
  LPB.pts_to_serialized_length
    (serialize_nlist (U64.v (Map?.len (Ghost.reveal xh)).value)
       (serialize_raw_data_item `serialize_nondep_then` serialize_raw_data_item))
    (to_slice v.cbor_serialized_payload);
  parse_nlist_kind_low (U64.v (Map?.len (Ghost.reveal xh)).value)
    (and_then_kind parse_raw_data_item_kind parse_raw_data_item_kind);
  assert_norm ((and_then_kind parse_raw_data_item_kind parse_raw_data_item_kind).parser_kind_low >= 1);
  FStar.Math.Lemmas.lemma_mult_le_right
    (U64.v (Map?.len (Ghost.reveal xh)).value) 1
    (and_then_kind parse_raw_data_item_kind parse_raw_data_item_kind).parser_kind_low;
  SZ.fits_lte
    (U64.v (Map?.len (Ghost.reveal xh)).value)
  ...
  fold (I.base_mixed_list_match_n cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) 0 (U64.v count) (pm_s /. 2.0R)
          (IT.Serialized #U64.t #cbor_map_entry 1.0R count (to_slice v.cbor_serialized_payload))
          (Map?.v (Ghost.reveal xh)));
  fold (I.mixed_list_match_n cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) 0 (U64.v count) (pm_s /. 2.0R)
          (IT.Base #U64.t #cbor_map_entry
             (IT.Serialized #U64.t #cbor_map_entry 1.0R count (to_slice v.cbor_serialized_payload)))
          (Map?.v (Ghost.reveal xh)));
  fold (I.mixed_list_match cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) (pm_s /. 2.0R)
          (IT.Base #U64.t #cbor_map_entry
             (IT.Serialized #U64.t #cbor_map_entry 1.0R count (to_slice v.cbor_serialized_payload)))
          (Map?.v (Ghost.reveal xh)));
  let ml : IT.mixed_list U64.t cbor_map_entry =
    IT.Base #U64.t #cbor_map_entry
      (IT.Serialized #U64.t #cbor_map_entry 1.0R count (to_slice v.cbor_serialized_payload));
  rewrite (I.mixed_list_match cbor_match_map_entry
             IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) (pm_s /. 2.0R)
             ...
             (Map?.v (Ghost.reveal xh)))
    as (I.mixed_list_match cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) (pm_s /. 2.0R) ml
          (Map?.v (Ghost.reveal xh)));
  Trade.intro_trade
    (I.mixed_list_match cbor_match_map_entry
       IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) (pm_s /. 2.0R) ml
    ...
    (PB.pts_to_parsed_strong_prefix
       (parse_nlist (U64.v (Map?.len (Ghost.reveal xh)).value)
          (nondep_then parse_raw_data_item parse_raw_data_item))
       (to_slice v.cbor_serialized_payload) #(pm_s /. 2.0R) (Map?.v (Ghost.reveal xh)))
    (pure (U64.v (Map?.len (Ghost.reveal xh)).value == 0 + U64.v count))
    fn _ {
      rewrite (I.mixed_list_match cbor_match_map_entry
                 IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) (pm_s /. 2.0R) ml
                 (Map?.v (Ghost.reveal xh)))
    ...
    (cbor_match pm (CBOR_Case_Map v) (Ghost.reveal xh))
{
  ...
  let count = IO.u64_ops.of_sizet (S.len v.cbor_map_ptr);
  ...
  fold (I.base_mixed_list_match_n cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item)
          0 (U64.v count) pm
          (IT.Slice #U64.t #cbor_map_entry v.cbor_map_array_perm v.cbor_map_payload_perm v.cbor_map_ptr count)
          (Map?.v (Ghost.reveal xh)));
  fold (I.mixed_list_match_n cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item)
          0 (U64.v count) pm
          ...
          (Map?.v (Ghost.reveal xh)));
  fold (I.mixed_list_match cbor_match_map_entry
          IO.u64_ops (nondep_then parse_raw_data_item parse_raw_data_item) pm
          (IT.Base #U64.t #cbor_map_entry
             (IT.Slice #U64.t #cbor_map_entry v.cbor_map_array_perm v.cbor_map_payload_perm v.cbor_map_ptr count))
          (Map?.v (Ghost.reveal xh)));
  let ml : IT.mixed_list U64.t cbor_map_entry =

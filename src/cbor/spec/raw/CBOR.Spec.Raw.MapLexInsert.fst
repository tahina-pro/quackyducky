...
(* Pure list-narrow helpers. These are definitionally identical to
   [LowParse.PulseParse.Iterator.list_narrow]/[list_narrow_split] (the shape used by
   the sorted-insert engine's postcondition), but restated here so this spec module
   stays in the pure spec layer (no dependency on the Pulse iterator). The Pulse
   layer bridges the two by definitional equality. *)
...
: Lemma
  (requires (
    ...
    kpos <= L.length l /\
    l_result == L.append (list_narrow l 0 kpos) (y :: list_narrow l kpos (L.length l - kpos))
  ))
  (ensures (
...
= let _ : squash (compare_prop order0 cbor_compare) = lemma_compare_prop in
  let _ = entry_lex_compare_prop in
  let l1 = list_narrow l 0 kpos in
  let l2 = list_narrow l kpos (L.length l - kpos) in
  // step a: l1 @ l2 == l
  list_narrow_split l kpos;

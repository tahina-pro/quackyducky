...
uint16_t iter_max(uint8_t *a, uint32_t len)
{
  ...
  uint8_t **r = KRML_HOST_MALLOC(sizeof (uint8_t *));
  r[0U] = a;
  uint32_t *r1 = KRML_HOST_MALLOC(sizeof (uint32_t));
  r1[0U] = len;
  uint16_t *r2 = KRML_HOST_CALLOC((uint32_t)1U, sizeof (uint16_t));
  KRML_CHECK_SIZE(sizeof (bool), (uint32_t)1U);
  bool *r3 = KRML_HOST_MALLOC(sizeof (bool));
  r3[0U] = len != (uint32_t)0U;
  while (r3[0U])
  {
    uint8_t *a1 = r[0U];
